...
// Use the same Product type as in product-service.ts
...
export default function InventorySummary() {
  ...
  const [online, setOnline] = useState(false) // Default to false for SSR

  // Handle online status with useEffect to avoid hydration mismatch
  useEffect(() => {
    setOnline(isOnline())

    // Optional: Add event listeners to update online status
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Fetch low stock products - returns Product[] directly
        const lowStockProducts = await fetchLowStockProducts()
        setLowStockProducts(lowStockProducts)

        // Fetch expiring products - returns Product[] directly
        const expiringProducts = await fetchExpiringProducts()
                    ...
                    <tbody>
                      {lowStockProducts.map((product) => (
                        <tr key={product.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="text-center py-2 px-2">{product.min_stock}</td>
